...
# wiki_filename = os.path.join(DATAIO_DIR, "People_Extract_Wikipedia.csv")
...
# grace_filename = os.path.join(DATAIO_DIR, "People_Extract_Grace.csv")
...
# df_merged.to_csv(os.path.join(DATAIO_DIR, "People_Load.csv"), index=False)

#STEP 3: Load the people summary records
class Command(BaseCommand):
    # Show this when the user types help
    help = "Loads data from people.csv into our Person model"

    def handle(self, *args, **options):
        if Person.objects.exists():
            print('People already loaded...but continuing.')
        else:
            print("Loading People for the first time")
        count = 0

        #This form of statement top ensure correct treatement of unusual unicode characters
        with open(os.path.join("D:\\Data", "TPAM", "People_Load.csv"), encoding="utf-8") as file:   

            for row in DictReader(file):
                # Exclude these names that are wrongly assigned from Grace's guide to Wikipedia equivalents    
                if row['gracetextslug'] not in [
                    ...
                    category = row['role'].replace('_', ' ')
                    role_fk, role_created = Role.objects.get_or_create(role=category,)     
                    if role_created == True:
                        print(role_fk, role_created, category)
                    person_fk.role.add(role_fk)