...
Wikipedia_file = os.path.join(
    DATAIO_DIR, 'Location_Stations_Closed_Wikipedia_Extract.csv')
Disused_file = os.path.join(DATAIO_DIR, 'Location_Stations_Disused_Extract.csv')
...
output_file = os.path.join(
    DATAIO_DIR, 'Location_Stations_Disused_Fuzzy_Analysis_Report.csv')
...
df_mapping = df_mapping.loc[pd.to_numeric(
    df_mapping['estimated_accuracy']) > 90]

df = df_wikipedia.set_index('Name').join(
    df_mapping.set_index('wikipedia_names'))
df.index.name = 'Name'
...
df.reset_index(inplace=True)
output_file = os.path.join(
    DATAIO_DIR, 'Location_Stations_Disused_Loadfile.csv')
