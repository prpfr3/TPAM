...
webpage = os.path.join(DATAIO_DIR, "List_of_Manufacturer.html")
...
try:
    ...
    df_clean.to_csv(os.path.join(DATAIO_DIR, "Manufacturer.csv"), index=False)
...
try:
    with connect(
        ...
        drop_table_query = "DROP TABLE locos_manufacturer"
        with connection.cursor() as cursor:
          cursor.execute(drop_table_query)

        create_manufacturer_query = """
        CREATE TABLE locos_manufacturer(
        ...
        with connection.cursor() as cursor:
          cursor.execute(create_manufacturer_query)
          connection.commit()
      ...
      ### INSERT manufacturer
        ...
        for i,row in df_clean.iterrows():
            sql = "INSERT INTO `locos_manufacturer` (`" +cols + "`) VALUES (" + "%s,"*(len(row)-1) + "%s)"

            with connection.cursor() as cursor:
                cursor.execute(sql, tuple(row))
                connection.commit()

        sql = "SELECT * FROM `locos_manufacturer`"
        with connection.cursor() as cursor:        
          cursor.execute(sql)
