                ...
                for row in DictReader(file):
                    if row["2"] == "Operators":
                        try:
                            ...
                            print(
                                row["0"],
                                " not found in the Lococlass wikipedia_name_slug field",
                            )
                        else:
                            for column in [
                                "4",
                                ...
                                if "wiki" in row[column]:
                                    try:
                                        ...
                                        c = Company.objects.get(wikislug=slug)
                                    except ObjectDoesNotExist:
                                        print(
                                            f"The owner_operator company {slug} for class {l} is not in the database"
                                        )
                                    except Exception as e:
                                        print(row[column], e)
                                    else:
                                        try:
                                            l.owner_operators.add(c)
                                        except Exception as e:
                                            print(row[column], e)
