...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        REQUEST_HEADERS = {
            "User-Agent": "LocoClass_W1_Scrape_Names.py; https://github.com/prpfr3/TPAM)"
        }

        classnames_all_output_file = os.path.join(
            DATAIO_DIR, "Class_All_W1_ClassNames_Full.csv"
        )
        classnames_all_csv_file = open(
            classnames_all_output_file, "wt+", newline="", encoding="utf-8"
        )
        classnames_all_output = csv.writer(classnames_all_csv_file)

        classnames_all_csvrow = []
        classnames_all_csvrow.append("category")
        classnames_all_csvrow.append("wikislug")
        classnames_all_csvrow.append("name")
        classnames_all_output.writerow(classnames_all_csvrow)

        classnames_dedup_output_file = os.path.join(
            DATAIO_DIR, "Class_All_W1_ClassNames_Deduplicated.csv"
        )
        classnames_dedup_csv_file = open(
            classnames_dedup_output_file, "wt+", newline="", encoding="utf-8"
        )
        classnames_dedup_output = csv.writer(classnames_dedup_csv_file)

        classnames_dedup_csvrow = []
        classnames_dedup_csvrow.append("wikislug")
        classnames_dedup_csvrow.append("name")
        classnames_dedup_output.writerow(classnames_dedup_csvrow)

        Categories = [
            "https://en.wikipedia.org/wiki/Category:Standard_gauge_steam_locomotives_of_Great_Britain",
        ...
        class_exclusions = [
            ...
            "/wiki/BR Standard Class 7 70048 The Territorial Army 1908–1958",
        ]
        ...
        for url in Categories:
            ...
            for link in soup.find_all(title=True):
                ...
                if (
                    not any(x in href for x in string_exclusions)
                    and href not in class_exclusions
                    and "/wiki" in href
                ):
                    try:
                        ...
                        classnames_all_csvrow = []
                        classnamearray = url.split(":", 2)
                        classnames_all_csvrow.append(classnamearray[2])
                        classnames_all_csvrow.append(href.replace("/wiki/", ""))
                        classnames_all_csvrow.append(link.get("title"))
                        classnames_all_output.writerow(classnames_all_csvrow)
                        ...
                        if href in unique_hrefs:
                            ...
                            classnames_dedup_csvrow = []
                            classnames_dedup_csvrow.append(href.replace("/wiki/", ""))
                            classnames_dedup_csvrow.append(link.get("title"))
                            classnames_dedup_output.writerow(classnames_dedup_csvrow)
        ...
        classnames_all_csv_file.close()
        classnames_dedup_csv_file.close()
