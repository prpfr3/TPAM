...
webpage = os.path.join(DATAIO_DIR, "List_of_Manufacturers.html")
...
try:
    ...
    df_clean.to_csv(os.path.join(DATAIO_DIR, "Manufacturers.csv"), index=False)
    print(df_clean.info())
    print(df_clean.head())
...
try:
    with connect(
        ...
        drop_table_query = "DROP TABLE locos_manufacturers"
        with connection.cursor() as cursor:
          cursor.execute(drop_table_query)

        create_manufacturers_query = """
        CREATE TABLE locos_manufacturers(
        ...
        with connection.cursor() as cursor:
          cursor.execute(create_manufacturers_query)
          connection.commit()
      ...
      ### INSERT manufacturers
        ...
        for i,row in df_clean.iterrows():
            sql = "INSERT INTO `locos_manufacturers` (`" +cols + "`) VALUES (" + "%s,"*(len(row)-1) + "%s)"

            with connection.cursor() as cursor:
                cursor.execute(sql, tuple(row))
                connection.commit()

        sql = "SELECT * FROM `locos_manufacturers`"
        with connection.cursor() as cursor:        
          cursor.execute(sql)
